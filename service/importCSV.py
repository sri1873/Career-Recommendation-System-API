...
def careerFit(studentId):
  ...
  career_subjects = []
  with open("C:/Users/grvn1/OneDrive/Desktop/Capstone/CareerJobData.csv", 'r', encoding='utf-8') as csv_file:
  ...
  updated_marks_dict = {subject: updated_mark for subject, updated_mark in updated_marks.items() if subject}
  headers_updated_marks = career_subjects[0][1:]

  updated_marks_values = [updated_marks_dict.get(subject, 0.0) for subject in headers_updated_marks]
  ...
  if recommended_job_roles:
        ...
        top_3_recommendations = [{"Job Role": job, "Percentage Similarity": similarity} for job, similarity in sorted_recommendations]
        return json.dumps(top_3_recommendations, indent=4)
  else:
...
def calculate_student_skill_gap(studentId):
    ...
    # Extract subjects for the student's job role using the found index
    job_role_subjects = {subject: float(mark) for subject, mark in zip(career_subjects[0][1:], career_subjects[job_role_index][1:])}
    ...
    for subject, mark in filtered_result_list2:
        if subject in career_subject_set:
            career_subject_index = career_subjects[0].index(subject)
            career_subject_weight = float(career_subjects[job_role_index][career_subject_index])
            updated_mark = (mark / 100) * career_subject_weight
            updated_marks[subject] = updated_mark
    ...
    for subject, updated_mark in updated_marks.items():
        if subject in job_role_subjects:
            ...
            if expected_marks != 0:  # Avoid division by zero
...
def calculate_student_overall_performance(studentId):
    ...
    overall_performance = sum(
        (expected_marks * (100 - ((expected_marks - updated_mark) / expected_marks) * 100)) / 100
        for subject, updated_mark in updated_marks.items() if subject in job_role_subjects and updated_mark != 0
    )
